...
fn mul(x: &Vec<Vec<Mint>>, y: &Vec<Vec<Mint>>) -> Vec<Vec<Mint>> {
    let n = x.len();
    let mut z = vec![vec![Mint::from(0); n]; n];
    for i in 0..n {
        for j in 0..n {
            for k in 0..n {
                z[i][j] += x[i][k] * y[k][j];
...
fn pow(x: &Vec<Vec<Mint>>, k: u64) -> Vec<Vec<Mint>> {
    if k == 1 {
        x.clone()
    } else if k % 2 > 0 {
        mul(x, &pow(x, k-1))
...
fn main() {
    ...
    let mut dp = vec![vec![vec![0; 1<<r]; r]; r];
    for i in 0..r {
        dp[i][i][1<<i] = 1;
    }
    for i in 0..1<<r {
        for j in 0..r {
            for k in 0..r {
                for l in 0..r {
                    if i>>l&1 > 0 { continue }
                    if g[k][l] == 0 { continue }
                    dp[j][l][i|1<<l] += dp[j][k][i];
                }
            }
        }
    }

    let mut f = vec![vec![Mint::from(0); r]; r];
    for i in 0..r {
        for j in 0..r {
            f[i][j] = dp[i][j].iter().sum::<usize>().into();
        }
    }

    let x = pow(&f, h as u64);
