...
fn main() {
    ...
    let mut b = vec![vec![]; a.len()];
    for &(x, r) in &xs {
        b[a.binary_search(&(x+r)).unwrap()].push(a.binary_search(&(x-r)).unwrap());
    }

    let mut st = Segtree::<Max<usize>>::new(a.len());
    for bi in b {
        let mut u = vec![];
        for &x in &bi {
            let k = st.prod(x+1, a.len());
            u.push((x, k+1));
        }
        for (x, k) in u {
            let k = k.max(st.get(x));
            st.set(x, k);
        }
    }
