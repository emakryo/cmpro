...
fn main() {
    ...
    let mut dp = vec![vec![[Mint::from(0); 3]; k+1]; n];
    dfs(0, usize::max_value(), &g, &mut dp);
    dbg!(&dp);

    println!("{}", dp[0][k][0] + dp[0][k][1] + dp[0][k][2]);
}

fn dfs(u: usize, p: usize, g: &Vec<Vec<usize>>, dp: &mut Vec<Vec<[Mint; 3]>>) {
    let k = dp[0].len();
    ...
    for &v in &g[u] {
        if v == p { continue }
        dfs(v, u, g, dp);
        let mut next = vec![[Mint::from(0); 3]; k];

        for i in 0..k {
            for j in 0..k {
                if i+j < k {
                    next[i+j][0] += dp[u][i][0] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]);
                    next[i+j][1] += dp[u][i][1] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]);
                    next[i+j][2] += dp[u][i][2] * (dp[v][j][0] + dp[v][j][1] + dp[v][j][2]);
                }

                if i+j+1 < k {
                    next[i+j+1][1] += dp[u][i][0] * dp[v][j][0];
                }
                if i+j < k {
                    next[i+j][1] += dp[u][i][0] * dp[v][j][1];
                    next[i+j][2] += dp[u][i][1] * dp[v][j][0];
                }
                if i+j >= 1 && i+j-1 < k {
                    next[i+j-1][2] += dp[u][i][1] * dp[v][j][1];
                }
            }
        }
