...
fn dfs(u: usize, p: usize, g: &Vec<Vec<usize>>, dp: &mut Vec<(Mint, usize)>, comb: &Combination<Mint>) {
    let mut c = 0;
    dp[u].0 = Mint::from(1);
    for &v in &g[u] {
        if v == p { continue }
        dfs(v, u, g, dp, comb);
        c += dp[v].1;
        dp[u].0 = dp[u].0 * dp[v].0 * comb.calc(c, dp[v].1);
    }
    dp[u].1 = c + 1;
}

fn dfs2(u: usize, p: usize, px: Mint, g: &Vec<Vec<usize>>, dp: &Vec<(Mint, usize)>, comb: &Combination<Mint>) -> Mint {
    let mut ret = 0.into();

    let mut x = px;
    let mut c = g.len() - dp[u].1;
    for &v in &g[u] {
        if v == p { continue }
        c += dp[v].1;
        x *= dp[v].0 * comb.calc(c, dp[v].1);
    }

    for &v in &g[u] {
        if v == p { continue }
        ret += x / comb.calc(c, dp[v].1) * comb.calc(c-1, dp[v].1-1);
        ret += dfs2(v, u, x / dp[v].0 / comb.calc(c, dp[v].1), g, dp, comb);
    }
...
fn main() {
    ...
    let mut dp = vec![Default::default(); n];
    let comb = Combination::new(200000);
    dfs(0, usize::max_value(), &g, &mut dp, &comb);
    let ans = dfs2(0, usize::max_value(), Mint::from(1), &g, &dp, &comb);
