...
use std::{fmt::Debug, ops::{Add, AddAssign, Mul, MulAssign}};
macro_rules! dbg {
    ($($xs:expr),+) => {
...
fn main() {
    ...
    let a = vec![Mint::from(1); k];
    let c = vec![Mint::from(1); k];

    let ans = kitamasa(&a, &c, n);

    println!("{}", ans);
}

fn kitamasa<T>(a: &[T], c: &[T], n: usize) -> T 
where T: Clone + Copy + Mul<Output=T> + Add<Output=T> + From<usize>
{
    let k = a.len();
    assert_eq!(k, c.len());
