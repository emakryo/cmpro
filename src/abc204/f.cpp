...
vector<vector<mint>> mul(const vector<vector<mint>> &x, const vector<vector<mint>> &y){
    int n=x.size();
...
vector<vector<mint>> pow(vector<vector<mint>> &x, ll n){
    if(n==1) return vector<vector<mint>>(x);
    if(n%2) return mul(x, pow(x, n-1));
    vector<vector<mint>> z = pow(x, n/2);
    return mul(z, z);
}
...
int main() {
    ...
    vector<vector<mint>> a((1<<h), vector<mint>((1<<h)));
    for(int s=0; s<(1<<h); s++){
        for(int t=0; t<(1<<h); t++){
            int ns = s^((1<<h)-1);
            int nt = t^((1<<h)-1);
    ...
    for(int i=0; i<(1<<h); i++) dbg(a[i]);
    if(w==1){
        cout << a[0][0] << endl;
        return 0;
    }

    vector<vector<mint>> z = pow(a, w);
    for(int i=0; i<(1<<h); i++) dbg(z[i]);
