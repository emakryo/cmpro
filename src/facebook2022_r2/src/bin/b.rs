...
fn solve() {
    proconio::input! {
        n: usize,
        k: usize,
        mut cs: [(i64, i64, i64, i64); n], // a, b, x, y
    }

    cs.sort_by_key(|&(_a, b, _x, _y)| b);

    let mut dp = vec![BinaryHeap::<Reverse<i64>>::new(); n];
    let mut d2b = BTreeMap::<i64, Vec<usize>>::new();
    for (i, &(a, b, x, y)) in cs.iter().enumerate() {
        d2b.entry(b).or_default().push(i);
    }

    let mut cum = BTreeMap::<i64, Vec<(Reverse<i64>, i64)>>::new();

    for (i, &(a, b, x, y)) in cs.iter().enumerate() {

        if !cum.contains_key(&a) {
            let c = cum.entry(a).or_default();
            for &j in d2b.get(&a).unwrap_or(&vec![]) {
                for &x in dp[j].iter() {
                    c.push((Reverse(x.0 - cs[j].3), cs[j].3));
                }
                c.push((Reverse(-cs[j].3), cs[j].3));
            }
            c.sort();
            c.truncate(1000*k);
        }

        let c = cum.get(&a).unwrap();
        for &z in c.iter() {
            if z.1 < x {
                dp[i].push(Reverse(z.0.0 + x));
                while dp[i].len() > k {
                    dp[i].pop();
                }
            }
    ...
    let mut all = BinaryHeap::<Reverse<i64>>::new();
    for zs in &dp {
        for z in zs {
            all.push(*z);
            if all.len() > k {
