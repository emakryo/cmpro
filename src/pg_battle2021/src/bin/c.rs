...
fn main() {
    ...
    let m = 500000;
    let mut dp = vec![Mint::from(0); m];
    let mut dp1 = vec![Mint::from(0); m];
    let mut dpk = vec![Mint::from(0); m];
    for i in (1..m-1).rev() {
        if n < i {
            dp[i] = Mint::from(i);
        } else {
            let s1 = dp1[i+1] - dp1[i+r[i-1]+1];
            let sk = dpk[i+1] - dpk[i+r[i-1]+1];
            dp[i] = (s1 * (r[i-1]+i+1) - sk) * 2 / r[i-1] / (r[i-1]+1);
        }

        dp1[i] = dp[i] + dp1[i+1];
        dpk[i] = dp[i] * i + dpk[i+1];
    }
