...
fn main() {
    ...
    let s = s.iter().map(|x| Mint::from((*x-b'0') as u64)).collect::<Vec<_>>();

    let mut ans = Mint::from(*s.last().unwrap());
    let mut c = Mint::from(1);
    let mut d = Mint::from(1);
    for a in s.into_iter().rev().skip(1) {
        c = c * 10 + d;
        d *= 2;
