...
int main() {
    ...
    vector<vector<mint>> xsum(dp), ysum(dp), dsum(dp);
    dp[1][1] = 1;

    for(int i=0; i<h; i++){
        for(int j=0; j<w; j++){
            if(s[i][j]=='.'){
                dp[i+1][j+1] += xsum[i][j+1] + ysum[i+1][j] + dsum[i][j];
                xsum[i+1][j+1] = xsum[i][j+1] + dp[i+1][j+1];
                ysum[i+1][j+1] = ysum[i+1][j] + dp[i+1][j+1];
                dsum[i+1][j+1] = dsum[i][j] + dp[i+1][j+1];
            }
        }
    }
