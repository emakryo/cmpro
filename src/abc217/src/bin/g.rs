...
fn main() {
    ...
    let comb = Combination::<Mint>::new(5005);
    let mut dp = vec![Mint::from(0); n+1];
    dp[cnt[0]] = 1.into();
    for i in 1..m {
        let mut next = vec![Mint::from(0); n+1];
        for j in 0..n {
            if dp[j] == Mint::from(0) { continue }
            for k in 0..=cnt[i] {
                if k <= j && j+cnt[i]-k <= n {
                    next[j+cnt[i]-k] += dp[j] * comb.calc(cnt[i], k) * comb.permutation(j, k);
                }
            }
        }
        dp = next;
...
pub mod combination {
    ...
    impl<M: ModIntBase> Combination<M> {
        ...
        pub fn permutation(&self, n: usize, k: usize) -> M {
            if n < k {
                0.into()
            } else if n < self.fact.len() {
