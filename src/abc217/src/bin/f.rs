...
fn main() {
    ...
    let mut dp = vec![vec![Mint::from(0); 2*n+1]; 2*n+1];
    for i in 0..=2*n {
        dp[i][i] = 1.into();
    }

    let mut ys = vec![BTreeSet::new(); 2*n];
    for (a, b) in xs {
        ys[a].insert(b);
    }

    let mut f = vec![Mint::from(1); 500];
    for i in 1..f.len() {
        f[i] = f[i-1] * i;
    }
    let c = |n, m| f[n] / f[n-m] / f[m];

    for w in (2..=2*n).step_by(2) {
        for i in 0..=2*n-w {
            for j in (1..w).step_by(2) {
                if ys[i].contains(&(i+j)) {
                    dp[i][i+w] = dp[i][i+w] + dp[i+1][i+j] * dp[i+j+1][i+w] * c(w/2, (j+1)/2);
                }
            }
            dbg!(i, i+w, dp[i][i+w]);
        }
