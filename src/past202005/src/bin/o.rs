...
fn main() {
    ...
    let mut mcf = MinCostFlowGraph::new(n+5);
    ...
    for i in 0..3 {
        mcf.add_edge(s, i+2, m, 0);
    }

    for j in 0..n {
        for i in 0..3 {
            mcf.add_edge(i+2, j+5, 1, inf - a[j] * b[j].pow((i+1) as u32) % r[i]);

        }
        mcf.add_edge(j+5, t, 1, a[j]*b[j]);
        mcf.add_edge(j+5, t, 1, a[j]*b[j]*b[j]-a[j]*b[j]);
        mcf.add_edge(j+5, t, 1, a[j]*b[j]*b[j]*b[j]-a[j]*b[j]*b[j]);
    }

    let flow = mcf.flow(s, t, 3 * m);

    println!("{}", 3*m*inf - flow.1);
}
