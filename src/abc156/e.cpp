...
int main(){
	...
	if(K>=N){
		mint ans(1);
		for(int i=0; i<N; i++) ans *= mint(i+1).inv() * (2*N-1-i);
		cout << ans.x << endl;
		return 0;
	}

	vector<mint> fact(1000000);
	fact[0] = mint(1);
	for(int i=1; i<1000000; i++) fact[i] = fact[i-1]*i;
	mint ans(0);

	for(int j=0; j<=K; j++){
		ans += fact[N]*fact[j].inv()*fact[N-j].inv() * fact[N-1]*fact[j].inv()*fact[N-1-j].inv();
		debug(fact[1].x, fact[100].x, ans.x);
	}
