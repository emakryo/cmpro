...
fn main() {
    proconio::input! {
        ...
        xs: [(Usize1, Usize1, Usize1, Usize1); n],
    }

    let mut f = MfGraph::<u32>::new(2+h+w+2*n);
    let (s, t) = (0, 1);
    ...
    for i in 0..w {
        f.add_edge(2+h+i, t, 1);
    }

    for (i, (a, b, c, d)) in xs.into_iter().enumerate() {
        for x in a..=c {
            f.add_edge(2+x, 2+h+w+i, 1);
        }

        for y in b..=d {
            f.add_edge(2+h+w+n+i, 2+h+y, 1);
        }
        f.add_edge(2+h+w+i, 2+h+w+n+i, 1);
    }
