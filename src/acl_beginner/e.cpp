...
struct LazySegmentTree{
    ...
    E propagate(E before, E change) const { return change; }
    T apply(T val, E laz) const { return {val.first, val.first*laz}; }
};
...
int main() {
    ...
    LazySegmentTree<pair<mint<>, mint<>>, int> st(init, {0, 0}, 0);
    for(int i=0; i<q; i++){
        int l, r, d; cin >> l >> r >> d;
        st.update(l-1, r, d);
        cout << st.query(0, n).second << endl;
    }
    return 0;
}