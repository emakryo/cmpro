...
int main() {
	...
	for(int i=0; i<n; i++) for(int j=0; j<6; j++){
		xs.push_back({a[i][j], i});
	}

	sort(xs.rbegin(), xs.rend());
	SegmentTree<double> st(n, 0);

	for(int i=0; i<xs.size(); i++){
		double e = st.query(0, n);
		double f = st.query(xs[i].second, xs[i].second+1);
		st.update(xs[i].second, f+(1+e)/6);
	}

	cout << setprecision(20) << st.query(0, n)+1 << endl;
