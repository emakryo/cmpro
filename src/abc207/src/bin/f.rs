...
fn main() {
    ...
    let mut dp = vec![vec![vec![]; 3]; n];

    dfs(0, None, &mut dp, &g);

    for i in 0..=n {
        println!("{}", dp[0][0][i]+dp[0][1][i]+dp[0][2][i]);
    }
}

fn dfs(u: usize, p: Option<usize>, dp: &mut DP, g: &G) {
    dp[u][0] = vec![1.into(), 0.into()];
    dp[u][2] = vec![0.into(), 1.into()];
    let mut total = vec![0.into(), 1.into()];
    for &v in &g[u] {
        if Some(v) == p { continue }
        dfs(v, Some(u), dp, g);
        let mut xs = dp[v][0].clone();
        for i in 0..xs.len() {
            xs[i] += dp[v][1][i];
        }
        
        dp[u][0] = convolution_naive(&dp[u][0], &xs);
        for i in 0..xs.len() {
            xs[i] += dp[v][2][i];
        }
        total = convolution_naive(&total, &xs);

        let mut ys = dp[v][1].clone();
        let k = ys.len();
        ys[0] += dp[v][2][0];
        for i in 1..k {
            ys[i] += dp[v][2][i] + dp[v][0][i-1];
        }
        dp[u][2] = convolution_naive(&dp[u][2], &ys);
    }

    for i in 0..(total.len()-1) {
        total[i+1] -= dp[u][0][i];
    }
    dp[u][1] = total;
}

fn convolution_naive(a: &[Mint], b: &[Mint]) -> Vec<Mint> {
    let (n, m) = (a.len(), b.len());
