...
fn main() {
    proconio::input!{
        n: usize,
        q: usize,
        es: [(Usize1, Usize1, usize, i64); n-1],
        qs: [(usize, i64, Usize1, Usize1); q],
    }
    ...
    let mut ans = vec![0; q];
    let lca = LcaTree::new(h);
    let mut vq = vec![vec![]; n];
    let mut aq = vec![vec![]; n];

    for (i, &(x, y, u, v)) in qs.iter().enumerate() {
        vq[u].push((i, x, y));
        vq[v].push((i, x, y));
        aq[lca.lca(u, v)].push((i, x, y));
        ans[i] += lca.dist(u, v);
    }

    fn dfs(u: usize, p: Option<usize>, g: &Vec<Vec<(usize, usize, i64)>>, vq: &Vec<Vec<(usize, usize, i64)>>, aq: &Vec<Vec<(usize, usize, i64)>>, ans: &mut Vec<i64>, dist: &mut Vec<i64>, count: &mut Vec<i64>) {
        for &(q, x, y) in &vq[u] {
            ans[q] += y * count[x] - dist[x];
        }
        for &(q, x, y) in &aq[u] {
            ans[q] -= 2 * y * count[x] - 2 * dist[x];
        }
        for &(v, c, w) in &g[u] {
            ...
            dfs(v, Some(u), g, vq, aq, ans, dist, count);
    ...
    dfs(0, None, &g, &vq, &aq, &mut ans, &mut dist, &mut count);
