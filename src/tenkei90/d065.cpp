...
int main() {
    ...
    int r, g, b, k, x, y, z;
    cin >> r >> g >> b >> k >> x >> y >> z;
    int s=k-y, t=k-z, u=k-x;

    vector<mint> xr(r+1), xg(g+1), xb(b+1);

    for(int i=s; i<=r; i++) xr[i] = c(r, i);
    for(int i=t; i<=g; i++) xg[i] = c(g, i);
    for(int i=u; i<=b; i++) xb[i] = c(b, i);
    dbg(xr);
    dbg(xg);
    dbg(xb);

    vector<mint> xrg = convolution(xr, xg);

    mint ans = 0;

    for(int i=0; i<=b; i++){
        if(0<=k-i&&k-i<=r+g) ans += xb[i]*xrg[k-i];
    }
