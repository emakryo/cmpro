...
fn main () {
    ...
    let mut st = LazySegtree::<MaxAdd>::from(vec![0; m]);
    let mut i = 0;
    let mut ans = 0;
    for j in 0..n {
        while xs[i].0 < xs[j].0 - k as i64 {
            st.apply_range(0.max(xs[i].1-k) as usize, m.min((xs[i].1+1) as usize), -1);
            i += 1;
        }

        st.apply_range(0.max(xs[j].1-k) as usize, m.min((xs[j].1+1) as usize), 1);
        ans = ans.max(st.all_prod());
    }
    println!("{}", ans);
}
