...
fn main() {
    ...
    if k == 1 {
        ...
        fn mul(x: &[Mint], y: &[Mint]) -> Vec<Mint> {
            let n = 2;
            let mut z = vec![Mint::from(0); n*n];
            for i in 0..n {
                for j in 0..n {
                    for k in 0..n {
        ...
        let m = vec![Mint::from(1), Mint::from(1), Mint::from(1), Mint::from(0)];
        let p = pow(&m, n);
        println!("{}", p[0]+p[1]);
