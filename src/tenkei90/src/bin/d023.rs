...
fn main() {
    ...
    let check = |p, s| {
        for i in 0..(w+2) {
            if i+w < w+2 && s>>i&s>>(i+w)&1 > 0 { return false; } // lower
            let x = (i+p+w-1)%w;
            if x != w-1 && s>>i&s>>(i+1)&1 > 0 { return false; }  // right
            if x > 0 && s>>i&s>>(i+w-1)&1 > 0 { return false; } // lower-left
            if x != w-1 && s>>i&s>>(i+w+1)&1 > 0 { return false; } // lower-right
        }
        true
    };

    let mut trans = vec![vec![]; w];
    let mut states: HashMap<(usize, usize), usize> = HashMap::new();

    let get_state = |p, s, states: &mut HashMap<(usize, usize), usize>, trans: &mut Vec<(Option<usize>, Option<usize>)>| {
        if let Some(i) = states.get(&(p, s)) {
            *i
        } else {
            let i = trans.len();
            states.insert((p, s), i);
            trans.push((None, None));
            i
        }
    };

    let mut que = VecDeque::new();
    que.push_back((0, 0usize));
    while let Some((p, s)) = que.pop_front() {
        if !check(p, s) { continue }
        let i = get_state(p, s, &mut states, &mut trans[p]);
        let q = (p+1)%w;
        let j0 = get_state(q, s>>1, &mut states, &mut trans[q]);
        if trans[p][i].0.is_none() {
            trans[p][i].0 = Some(j0);
            que.push_back((q, s>>1));
        }
        ...
        let t = (1<<(w+1))|s;
        if check(p, t) {
            let j1 = get_state(q, t>>1, &mut states, &mut trans[q]);
            if trans[p][i].1.is_none() {
                trans[p][i].1 = Some(j1);
                que.push_back((q, t>>1));
            }
        }
    ...
    let mut dp = vec![Mint::from(0); trans[0].len()];
    dp[0] = Mint::from(1);

    for i in 0..(h*w) {
        let mut next = vec![Mint::from(0); trans[(i+1)%w].len()];
        for k in 0..trans[i%w].len() {
            let t = trans[i%w][k];
            if let Some(t0) = t.0 {
                next[t0] += dp[k];
            }
