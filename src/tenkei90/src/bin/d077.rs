...
fn main() {
    ...
    for (i, (x, y)) in b.into_iter().enumerate() {
        bm.insert((x, y), i);
    }

    let mut f = MfGraph::new(2*n+2);
    let src = 0;
    let snk = 1;

    let mut es = vec![vec![None; 8]; n];
    for (i, (x, y)) in a.into_iter().enumerate() {
        f.add_edge(src, 2+i, 1);
        f.add_edge(2+n+i, snk, 1);
        let dx = [1, 1, 0, -1, -1, -1, 0, 1];
        let dy = [0, 1, 1, 1, 0, -1, -1, -1];
        for j in 0..8 {
            let bx = x + t * dx[j];
            let by = y + t * dy[j];
            if let Some(k) = bm.get(&(bx, by)) {
                let e = f.add_edge(2+i, 2+n+*k, 1);
                es[i][j] = Some(e);
            }
        }
    }

    let m = f.flow(src, snk);
    if m < n {
        println!("No");
        return;
    }
    ...
    for i in 0..n {
        let ans = es[i].iter().enumerate().filter_map(|(i, e)| {
            if let Some(e) = e {
                let e = f.get_edge(*e);
                if e.flow > 0 {
                    Some(i)
