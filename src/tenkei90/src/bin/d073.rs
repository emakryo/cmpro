...
fn main() {
    ...
    let c: Vec<_> = c.into_iter().map(|x| x[0]).collect();
    ...
    let mut dp = vec![(Mint::from(0), Mint::from(0)); n];
    fn dfs(u: usize, p: Option<usize>, dp: &mut Vec<(Mint, Mint)>, g: &Vec<Vec<usize>>, c: &Vec<u8>){
        dp[u] = (1.into(), 0.into());
        ...
        for &v in &g[u] {
            ...
            if c[u] == c[v] {
                dp[u] = (
                    dp[u].0 * dp[v].1 + dp[u].0 * dp[v].0,
                    dp[u].1 * dp[v].1 + (dp[u].0 + dp[u].1) * (dp[v].0 + dp[v].1) - dp[u].0 * dp[v].0
                );
            } else {
                dp[u] = (
                    dp[u].0 * dp[v].1,
                    dp[u].1 * dp[v].1 + (dp[u].0 + dp[u].1) * (dp[v].0 + dp[v].1)
                );
            }
        }
    }

    dfs(0, None, &mut dp, &g, &c);
