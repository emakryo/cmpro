...
fn main() {
    ...
    let mut ai = a.iter().enumerate().map(|(i, a)| (a, i)).collect::<Vec<_>>();
    ai.sort();
    let mut a = vec![0; n];
    for (i, &(_, j)) in ai.iter().enumerate() {
        a[j] = i;
    }

    dbg!(&a);

    let mut dp = Segtree::<Additive<Mint>>::new(n+1);
    dp.set(0, Mint::from(1));
    let mut cnt = Segtree::<Additive<usize>>::new(n+1);
    let mut t = 0;
    let mut j = 0;
