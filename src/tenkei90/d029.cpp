...
struct LazySegmentTree{
    ...
    E propagate(E before, E change) const { return max(before, change); }
    T apply(T val, E laz) const { return max(val, laz); }
};
...
int main() {
    ...
    LazySegmentTree<int, int> st(w+1, 0, 0);

    for(int i=0; i<n; i++){
        int x = st.query(l[i]-1, r[i]);
        cout << x+1 << endl;

        st.update(l[i]-1, r[i], x+1);
    }
    return 0;
}