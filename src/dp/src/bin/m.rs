...
fn main() {
    ...
    let mut dp = vec![vec![Mint::from(0); k+1]; n+1];
    dp[0][0] = 1.into();

    let mut cum = vec![Mint::from(0); k+2];
    for i in 0..n {
        for x in 0..k+1 {
            cum[x+1] = cum[x] + dp[i][x];
        }

        for x in 0..k+1 {
            dp[i+1][x] = cum[x+1] - cum[x.saturating_sub(a[i])];
        }
    }
...
fn main0() {
    ...
    let mut dp = FenwickTree::new(k+1, Mint::from(0));
    dp.add(0, Mint::from(1));

    for i in 0..n {
        for x in (0..=k).rev() {
            dp.add(x, dp.sum(x.saturating_sub(a[i]), x));
        }
    }

    println!("{}", dp.sum(k, k+1));
}
