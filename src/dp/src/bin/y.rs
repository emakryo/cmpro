...
fn main() {
    ...
    let c = Combination::<Mint>::new(300000);

    xs.push((1, 1));
    xs.push((h, w));

    xs.sort_by_key(|x| x.0+x.1);
    let mut dp = vec![[Mint::from(0); 2]; n+2];
    dp[0][0] = 1.into();

    for i in 1..n+2 {
        for j in 0..i {
            if xs[i].0 < xs[j].0 || xs[i].1 < xs[j].1 { continue }
            let x = xs[i].0 - xs[j].0;
            let y = xs[i].1 - xs[j].1;
            let p = c.calc(x+y, x) * dp[j][1];
            dp[i][0] += p;

            let q = c.calc(x+y, x) * dp[j][0];
            dp[i][1] += q;
        }
    }

    println!("{}", dp[n+1][1]-dp[n+1][0]);
