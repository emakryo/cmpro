...
fn main() {
    ...
    fn dfs0(u: usize, p: Option<usize>, g: &Vec<Vec<usize>>, dp: &mut Vec<ModInt>) {
        dp[u] = 1.into();
        for &v in &g[u] {
    ...
    let mut dp = vec![ModInt::default(); n];
    dfs0(0, None, &g, &mut dp);

    fn dfs1(u: usize, p: Option<usize>, g: &Vec<Vec<usize>>, dp: &Vec<ModInt>, x: ModInt, ans: &mut Vec<ModInt>) {
        let mut ls = vec![1.into()];
        let mut rs = vec![1.into()];
        let mut l: ModInt = 1.into();
        let mut r: ModInt = 1.into();
        for &v in &g[u] {
            if Some(v) != p {
                l *= dp[v]+1;
            }
        ...
        for (i, &v) in g[u].iter().enumerate() {
            if Some(v) == p { continue }
            let z = x * ls[i] * rs[g[u].len()-1-i] + 1;
            dfs1(v, Some(u), g, dp, z, ans);
        }
    }
    dbg!(&dp);

    let mut ans = vec![1.into(); n];
    dfs1(0, None, &g, &dp, 1.into(), &mut ans);
