...
fn main() {
    ...
    let a: M = a.into_iter().map(|ai| ai.into_iter().map(Mint::from).collect()).collect();

    let d = pow(&a, k);

    let ans: Mint = d.into_iter().map(|di| di.into_iter().sum::<Mint>()).sum();
