...
fn main() {
    ...
    for (l, r, a) in xs {
        *rs.entry((r, l)).or_insert(0) += a;
    }

    struct AddMax;
    impl MapMonoid for AddMax {
        type F = i64;
        type M = Max<i64>;
    ...
    let mut dp = LazySegtree::<AddMax>::new(n+1);
    dp.set(0, 0);
...
pub mod lazysegtree {
    ...
    impl<F: MapMonoid> LazySegtree<F> {
        ...
        pub fn to_vec(&mut self) -> Vec<<F::M as Monoid>::S> {
            let mut v = vec![];
            for i in 0..self.n {
                v.push(self.get(i));
            }
