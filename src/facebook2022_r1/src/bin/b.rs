...
fn solve() {
    ...
    let a: Mint = ps.iter().map(|&(a, _)| Mint::from(a)).sum();
    let b: Mint = ps.iter().map(|&(_, b)| Mint::from(b)).sum();
    let x: Mint = qs.iter().map(|&(x, _)| Mint::from(x)).sum();
    let y: Mint = qs.iter().map(|&(_, y)| Mint::from(y)).sum();

    let a2: Mint = ps.iter().map(|&(a, _)| Mint::from(a*a)).sum();
    let b2: Mint = ps.iter().map(|&(_, b)| Mint::from(b*b)).sum();
    let x2: Mint = qs.iter().map(|&(x, _)| Mint::from(x*x)).sum();
    let y2: Mint = qs.iter().map(|&(_, y)| Mint::from(y*y)).sum();

    let ans = a2 * q + x2 * n - a * x * 2 + b2 * q + y2 * n - b * y * 2;
    println!("{}", ans);
}
