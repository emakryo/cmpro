...
fn main() {
    ...
    let o = s.iter().flatten().filter(|&&x| x == b'o').count();

    let mut f = MinCostFlowGraph::new(n*m+2+o);
    ...
    for i in 0..n {
        for j in 0..m {
            if s[i][j] == b'o' {
                f.add_edge(src, 2+p, 1, 0);
                f.add_edge(2+p, 2+o+m*i+j, 1, (n+m) as i32);
                let mut dp = vec![vec![false; m]; n];
                dp[i][j] = true;
                for  k in i..n {
                    for l in j..m {
                        if s[k][l] != b'#' && ((k>0 && dp[k-1][l]) || (l>0 && dp[k][l-1])) {
                            dp[k][l] = true;
                            f.add_edge(2+p, 2+o+m*k+l, 1, (n+m-(k-i)-(l-j)) as i32);
                            // dbg!(2+p, 2+o+m*k+l, n+m-(k-i)-(l-j));
                        }
                    }
                }
                p += 1;
            ...
            f.add_edge(2+o+m*i+j, snk, 1, 0);
        }
    }

    let (f, c) = f.flow(src, snk, o as i32);
    dbg!(f, c);
    println!("{}", f * (n+m) as i32 - c);
}
