...
struct modint {
    ...
    modint<m> inv() const {
        return modint<m>(inv_gcd(x, m).second);
    }
};
...
std::vector<long long> convolution(std::vector<long long> a, std::vector<long long> b){
    ...
    std::vector<mint> a2(a.size()), b2(b.size());
    for(int i=0; i<a.size(); i++) a2[i] = mint(a[i]);
    for(int i=0; i<b.size(); i++) b2[i] = mint(b[i]);
