...
fn main() {
    ...
    let mut par = vec![0; n];
    fn dfs(u: usize, p: usize, g: &Vec<Vec<usize>>, depth: &mut Vec<usize>, par: &mut Vec<usize>) {
        for &v in &g[u] {
            if v == p { continue; }
            depth[v] = depth[u]+1;
            par[v] = u;
    ...
    dfs(0, usize::max_value(), &g, &mut depth, &mut par);
    ...
    let b = count.into_iter().skip(1).collect::<Vec<usize>>();
    let s = b.iter().sum::<usize>();
    let p = if (k+s as i64)%2 == 0 && (0..=s as i64).contains(&((k+s as i64)/2)) {
        ((k + s as i64) / 2) as usize
    } else {
        println!("0");
        return;
    };

    let mut dp = vec![Mint::from(0); p+1];
    let mut next = vec![Mint::from(0); p+1];
    dp[0] = 1.into();
