...
fn main() {
    ...
    let mut h = vec![vec![]; n]; // edges only to large degree nodes
    for i in 0..n {
        for &v in &g[i] {
            if g[v].len() > lim {
                h[i].push(v);
    ...
    let mut qx = vec![0];  // propageted node value when i-th query is executed.
    let mut t = vec![0; n]; // update for each nodes
    let mut s = vec![0; n]; // update to large nodes

    for (i, xi) in x.into_iter().enumerate() {
        let mut y = t[xi].max(s[xi]);
        ...
        dbg!(&t, y, qx.last().unwrap());
    }

    let mut ans = vec![];

    for i in 0..n {
        let mut y = t[i].max(s[i]);
        if g[i].len() <= lim {
            for &v in &g[i] {
                y = y.max(t[v]);
            }
