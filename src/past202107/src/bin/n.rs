...
fn main() {
    ...
    let mut ym = BTreeMap::new();
    for (i, y) in ys.iter().enumerate() {
        ym.insert(*y, i);
    }

    let mut begin = vec![vec![]; xd.len()];
    let mut end = vec![vec![]; xd.len()+1];
    for &(a, b, c, d) in &qs {
        let i = xm.get(&a).unwrap();
        let j = ym.get(&b).unwrap();
        let k = xm.get(&c).unwrap();
        let l = ym.get(&d).unwrap();
    ...
    struct XorSum(i64, i64);
    impl Monoid for XorSum {
        type S = Self;
        fn identity() -> Self::S {
            Self(0, 0)
        }
    ...
    impl MapMonoid for XorSum {
        type M = Self;
        type F = i64;
        fn identity_map() -> Self::F {
            0
        }
        fn mapping(f: &i64, x: &Self) -> Self {
            if *f==1 {
                Self(x.0, x.0 - x.1)
            } else {
                *x
    ...
    let mut st = LazySegtree::<XorSum>::new(yd.len());

    for i in 0..yd.len() {
        st.set(i, XorSum(yd[i], 0));
    }
    let mut ans = 0;
    for i in 0..xd.len() {
        for &(j, l) in &end[i] {
            st.apply_range(j, l, 1);
        }
        for &(j, l) in &begin[i] {
            dbg!(i, j, l);
            st.apply_range(j, l, 1);
