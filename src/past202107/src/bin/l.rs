...
fn main() {
    ...
    let mut cnt = BTreeMap::<usize, BTreeSet<usize>>::new();
    for (i, ai) in a.iter().enumerate() {
        if let Some(x) = cnt.get_mut(ai) {
            x.insert(i);
        } else {
    ...
    let mut st = Segtree::<Min<usize>>::from(a);

    for (t, x, y) in xs {
        let x = x-1;
        if t == 1 {
            let ai = st.get(x);
            let s = cnt.get_mut(&ai).unwrap();
            s.remove(&x);
            
            st.set(x, y);
            if let Some(s) = cnt.get_mut(&y) {
                s.insert(x);
            } else {
                cnt.insert(y, BTreeSet::from_iter(vec![x]));
            }
        } else {
            let p = st.prod(x, y);
            let s = cnt.get(&p).unwrap();
            let mut ans = vec![];
            for x in s.range(x..y) {
