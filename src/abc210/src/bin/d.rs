...
fn main() {
    ...
    let mut stx = vec![];
    for i in 0..h {
        let mut xs = a[i].clone();
        for j in 0..w {
            xs[j] += c*j as i64;
        }
        stx.push(LazySegtree::<AddMin>::from(xs));
    }
    ...
    for i in 0..w {
        let mut xs = vec![0; h];
        for j in 0..h {
            xs[j] = a[j][i] + c * j as i64;
        }
        sty.push(LazySegtree::<AddMin>::from(xs));
    }
    ...
    for i in 0..h {
        for j in 0..w {
            let x = stx[i].all_prod();
            let y = sty[j].all_prod();
            let x0 = stx[i].prod(0, j).min(stx[i].prod(j+1, w));
            let y0 = sty[j].prod(0, i).min(sty[j].prod(i+1, h));
            ans = ans.min((x+y0).min(x0+y));
            dbg!(x,y, x0,y0);
            stx[i].apply_range(0, j+1, c);
            stx[i].apply_range(j+1, w, -c);
        }
        for j in 0..w {
            sty[j].apply_range(0, i+1, c);
            sty[j].apply_range(i+1, h, -c);
        }
    }
