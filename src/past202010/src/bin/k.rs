...
fn main() {
    ...
    let mut inv = vec![0; k];
    let mut cnt = vec![[0; 21]; k];
    for i in 0..k {
        let mut f = FenwickTree::new(21, 0usize);
        for &aj in &a[i] {
            inv[i] += cnt[i].iter().sum::<usize>()-f.accum(aj+1);
            f.add(aj, 1);
            cnt[i][aj] += 1;
        } 
        dbg!(inv[i]);
