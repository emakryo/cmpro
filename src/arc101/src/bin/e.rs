...
fn main() {
    ...
    let mut h = vec![Mint::from(0); n+1];
    h[2] = 1.into();
    for i in 3..=n {
        h[i] = h[i-2] * (i-1);
    }

    let mut dp = vec![vec![[Mint::from(0); 2]; n]; n];

    fn dfs(u: usize, p: Option<usize>, g: &Vec<Vec<usize>>, h: &Vec<Mint>, dp: &mut Vec<Vec<[Mint; 2]>>) {
        dp[u] = vec![[0.into(); 2], [1.into(), 0.into()]];
        for &v in &g[u] {
            if Some(v) == p { continue }
            dfs(v, Some(u), g, h, dp);

            let mut next = vec![[Mint::from(0); 2]; dp[u].len()+dp[v].len()-1];
            for i in 0..dp[u].len() {
                for p in 0..2 {
                    for j in 0..dp[v].len() {
                        for q in 0..2 {
                            let x = dp[u][i][p] * dp[v][j][q];
                            next[i+j][(p+q)%2] += x;
                            next[i][(p+q+1)%2] += x * h[j];
                        }
                    }
                }
            }
    ...
    dfs(0, None, &g, &h, &mut dp);

    let mut ans = Mint::from(0);
    for i in 0..=n {
        ans += (dp[0][i][0] - dp[0][i][1]) * h[i];
    }
