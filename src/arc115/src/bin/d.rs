...
fn main() {
    ...
    let c = Combination::<Mint>::new(10000);
    ...
    for (a, b) in es {
        ...
        if uf.same(a, b) {
            ...
            let ea = e[uf.leader(a)];
            let eb = e[uf.leader(b)];
            e[uf.leader(a)] += eb;
            e[uf.leader(b)] += ea;
            uf.merge(a, b);
        }
    ...
    for i in 0..n {
        ...
        let ni = uf.size(i);
        let mut x = vec![Mint::from(0); ni+1];
        for k in (0..=ni).step_by(2) {
            x[k] = Mint::from(2).pow(e[i]) * c.calc(ni, k);
        }
