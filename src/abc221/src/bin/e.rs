...
fn main() {
    ...
    let a = a.iter().map(|x| c.binary_search(x).unwrap()).collect::<Vec<_>>();
    type Mint = ModInt998244353;

    impl internal_type_traits::Zero for Mint {
        fn zero() -> Self {
            Mint::from(0)
        }
    }
    ...
    impl MapMonoid for MulAdd {
        ...
        fn mapping(f: &Self::F, x: &Mint) -> Mint {
            *x * *f
        }
    }
    ...
    let mut dp = LazySegtree::<MulAdd>::new(c.len());
    let mut ans = Mint::from(0);
    for ai in a {
        ans += dbg!(dp.prod(0, ai+1));
        dp.apply_range(0, c.len(), 2.into());
        let x = dp.get(ai);
        // dbg!(ai, x);
        dp.set(ai, x+1);
    }

    // dbg!((0..c.len()).map(|i| dp.get(i)).collect::<Vec<_>>());
