...
fn main() {
    ...
    fn dfs(u: usize, p: usize, g: &Vec<Vec<usize>>, depth: &mut Vec<usize>) {
        for &v in &g[u] {
            if v == p { continue }
            depth[v] = depth[u] + 1;
            dfs(v, u, g, depth);
    ...
    let mut depth0 = vec![0; n];
    dfs(0, usize::max_value(), &g, &mut depth0);
    let mut u0 = 0;

    for i in 0..n {
        if depth0[i] > depth0[u0] {
            u0 = i;
        }
    }

    let mut depth = vec![0; n];
    dfs(u0, usize::max_value(), &g, &mut depth);

    let d = *depth.iter().max().unwrap();
    ...
    let mut depthv = vec![0; n];
    dfs(v0, usize::max_value(), &g, &mut depthv);

    fn dfs2(u: usize, p: usize, g: &Vec<Vec<usize>>, depth: &Vec<usize>, md: usize, count: &mut Vec<usize>) {
        if depth[u] == md {
            count[u] = 1;
            return;
        }
    ...
    if d % 2 == 0 {
        let mut c = 0;
        for i in 0..n {
            if depth[i] == d/2 && depthv[i] == d/2 {
                c = i;
            }
        }

        assert!(depth[c] == d/2 && depthv[c] == d/2);

        let mut depthc = vec![0; n];
        dfs(c, usize::max_value(), &g, &mut depthc);

        let mut count = vec![0; n];
        dfs2(c, usize::max_value(), &g, &depthc, d / 2, &mut count);
        ...
        assert!(depth[c0] == d/2 && depthv[c1] == d/2);

        let mut depthc0 = vec![0; n];
        let mut depthc1 = vec![0; n];

        dfs(c0, c1, &g, &mut depthc0);
        dfs(c1, c0, &g, &mut depthc1);

        let mut count0 = vec![0; n];
        let mut count1 = vec![0; n];

        dfs2(c0, c1, &g, &depthc0, d/2, &mut count0);
        dfs2(c1, c0, &g, &depthc1, d/2, &mut count1);

        let ans = Mint::from(count0[c0]) * count1[c1];
