...
fn main() {
    ...
    let mut stmax = Segtree::<Max<i64>>::new(n);
    let mut stmin = Segtree::<Min<i64>>::new(n);
    stmax.set(0, ps[0].1);
    stmin.set(0, ps[0].1);

    let mut ans = 0;

    for (i, &(xi, yi)) in ps[1..].iter().enumerate() {
        let mut lb = 0;
        let mut ub = inf;
        while ub - lb > 1 {
            let x = (lb + ub)/2;
            let k = match ps.binary_search(&(xi-x+1, -1)) {
                Ok(k) => k,
                Err(k) => k,
            };
            if k == 0 {
            ...
            let ymin = stmin.prod(0, k);
            let ymax = stmax.prod(0, k);
            if ymin <= yi-x || yi+x <= ymax {
                lb = x;
            } else {
                ub = x;
            }
