...
fn main() {
    ...
    dfs(0, None, &mut back, &mut visit, &g);

    let mut t = vec![vec![]; n];
    for u in 0..n {
        for &v in &g[u] {
            if !back.contains(&(u, v)) && !back.contains(&(v, u)) {
                t[u].push((v, 1));
            }
        }
    }

    let lca = LcaTree::new(t);
    ...
    let inf = 1000000000;
    let mut a = vec![vec![inf; vs.len()]; vs.len()];
    for &(u, v) in &back {
        a[vm[u]][vm[v]] = 1;
        a[vm[v]][vm[u]] = 1;
    }
    ...
    for i in 0..vs.len() {
        for j in 0..vs.len() {
            a[i][j] = a[i][j].min(lca.dist(vs[i], vs[j]));
        }
    }
    ...
    for (u, v) in qs {
        let mut dist = lca.dist(u, v);

        for i in 0..vs.len() {
            for j in 0..vs.len() {
                dist = dist.min(a[i][j] + lca.dist(vs[i], u) + lca.dist(vs[j], v));
            }
        }
...
fn dfs(u: usize, p: Option<usize>, back: &mut Vec<(usize, usize)>, visit: &mut Vec<bool>, g: &Vec<Vec<usize>>) {
    visit[u] = true;
    for &v in &g[u] {
...
impl<W: crate::internal_type_traits::Integral> LcaTree<W> {
    ...
    pub fn new_with_root(g: Vec<Vec<(usize, W)>>, root: usize) -> Self {
        let n = g.len();
        assert!(root<n);
        fn dfs<W: crate::internal_type_traits::Integral>(u: usize, p: Option<usize>, par: &mut Vec<usize>, depth: &mut Vec<usize>, dist: &mut Vec<W>,
            g: &Vec<Vec<(usize, W)>>) {
            for &(v, c) in &g[u] {
                if Some(v) == p { continue }
                par[v] = u;
