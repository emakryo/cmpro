...
fn solve() {
    ...
    let mut cf = [Mint::from(0); 3]; // FOX, count from front
    let mut cb = [Mint::from(0); 3]; // FOX, count from back
    let mut sf = Mint::from(0); // sum from front
    let mut sb = Mint::from(0); // sum from back
    let mut first = 0;
    let mut last = 0;
    let mut all = Mint::from(0);
    let mut len = Mint::from(0);
    let mut first_idx = Mint::from(0);
    let mut last_idx = Mint::from(0);
    ...
    for i in 0..k {
        match u[i] {
            ...
            b'.' => {
                // dbg!(sb, sf, cf, cb, all, len, first_idx, last_idx);
                ans = ans * 2 + sf * len + sb * len;

                if first != 0 && last != 0 && first != last {
                    ans += (len - first_idx) * (last_idx + 1);
                }

                cf[first] += len;
                cb[last] += len;
                let mut nsf = sf * 2 + len * all;
                let mut nsb = sb * 2 + len * all;
